...
$Lastupdatetime_XP = Get-Content E:\WsusDados\Repository_PowerShell\HML\DateLastApplyUpdate_XP.txt
...
($updates_XP | Select-Object -Last 1 creationdate | ft -HideTableHeaders  | Out-String).Trim() |Out-File E:\WsusDados\Repository_PowerShell\HML\DateLastApplyUpdate_XP.txt

# Listar os 50 updates para Windows 7
#$lastupdatetime_7 = "10/03/2009 00:00:00"
#Lendo a ultima data setada
$lastupdatetime_7 = Get-Content E:\WsusDados\Repository_PowerShell\HML\DateLastApplyUpdate_7.txt
...
Select-Object -First 50  KnowledgebaseArticles,CreationDate,ProductTitles
#Exportando dados para texto
($updates_7 | Select-Object -Last 1 creationdate | ft -HideTableHeaders  | Out-String).Trim() |Out-File E:\WsusDados\Repository_PowerShell\HML\DateLastApplyUpdate_7.txt
 

# Listar os 50 updates para Windows 8.1
#$lastupdatetime_81 = "08/10/2013 00:00:00"
#Lendo a ultima data setada
$lastupdatetime_81 = Get-Content E:\WsusDados\Repository_PowerShell\HML\DateLastApplyUpdate_81.txt
...
Select-Object -First 50  KnowledgebaseArticles, creationdate,ProductTitles
#Exportando dados para texto
($updates_81 | Select-Object -Last 1 creationdate | ft -HideTableHeaders  | Out-String).Trim() |Out-File E:\WsusDados\Repository_PowerShell\HML\DateLastApplyUpdate_81.txt

# Exportar os dados acima para o arquivo
($updates_XP + $Updates_7 + $Updates_81 | ft -HideTableHeaders KnowledgebaseArticles | Out-String).Trim()| Out-File E:\WsusDados\Repository_PowerShell\HML\UpdatesToInstall_HML.txt

$fileName = "UpdateList-" + (Get-Date -Format ddMMyyyy) + ".txt"
$location = "E:\WsusDados\Repository_PowerShell\HML\Log_KB_Installed"
($updates_XP + $Updates_7 + $Updates_81  | Out-String).Trim() | Out-file -append -FilePath (Join-Path $location $fileName)

# Substituindo caracteres
$file_HML = Get-Content E:\WsusDados\Repository_PowerShell\HML\UpdatesToInstall_HML.txt
$file_HML = $file_HML.Replace('{', 'KB')
$file_HML = $file_HML.Replace('}', '')
$file_HML = $file_HML.Replace('  ', '')
$file_HML | Out-File E:\WsusDados\Repository_PowerShell\HML\UpdatesToInstall_HML.txt

#Aplicando Updates aos grupos no WSUS
foreach ($f_HML in $file_HML) {
$update = $wsus.SearchUpdates($f_HML) # Nome do KB ou atualização
